...
Fixed Beaver Bother is a mod for Donkey Kong 64: Recompiled that adjusts how beavers behave in Beaver Bother. The changes are as follows:
- Adding a 20% chance for any beavers in beaver bother to be gold
   - Gold beavers are worth 2 points
   - This 20% chance is rolled upon the actor being spawned
- Increasing the beaver's scare duration
   - This is slightly reworked from Rando to fix a minor oddity with that version of the beaver bother fix.
- Giving the beavers noclip (temporarily) to make them actually go down the hole

## Installation
1. Download the latest `fixed_beaver_bother.nrm` from Releases.
...
The produced mod file is named `fixed_beaver_bother.nrm`.
